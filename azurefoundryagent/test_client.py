...
async def print_detailed_response(
    ...
    try:
        ...
        # Extract from result field (A2A response structure)
        ...
        # Get agent's response from status message
        ...
        # Also check history for the latest agent message
            ...
            for msg in reversed(history):
                if msg.get('role') == 'agent' and msg.get('parts'):
                    for part in msg['parts']:
                        if part.get('kind') == 'text':
                            text = part.get('text', '')
                            if text and text != 'Processing your request...':
                                logger.info(f'   💬 Latest response: {text}')
                                break
                    break

    except Exception as e:
        logger.error(f'Could not parse response details: {e}')
